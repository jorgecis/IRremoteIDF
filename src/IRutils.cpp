...
#ifndef UNIT_TEST
/// Print a uint64_t/unsigned long long to stdout (i.e. the console UART).
/// printf() has no portable 64 bit format specifier across all targets.
/// @param[in] input The value to print
/// @param[in] base The output base.
void serialPrintUint64(uint64_t input, uint8_t base) {
  fputs(uint64ToString(input, base).c_str(), stdout);
}
#endif  // UNIT_TEST
